<template>
  <div
    ...
    <div class="w-full max-w-4xl flex items-center justify-center">
      <div class="flex flex-col md:flex-row items-stretch w-full flex-1 rounded-3xl overflow-hidden shadow-2xl min-h-[636px]">

        <!-- Right Side - Hero Image -->
        <div class="hidden md:flex w-1/2 bg-gradient-to-br relative from-teal-600 to-cyan-500 text-white items-center justify-center p-12">
            <div class="absolute inset-0" style="background: linear-gradient(135deg, #0d9488 0%, #06b6d4 100%);"/>
            <div class="relative z-10 p-12 flex flex-col items-center justify-center text-white">
              <div class="text-center space-y-6">
                ...
                <div class="flex items-center justify-center gap-4 mt-8">
                  <div class="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl rotate-12 animate-float"/>
                  <div class="w-20 h-20 bg-white/30 backdrop-blur-sm rounded-3xl -rotate-6 animate-float-delayed"/>
                  <div class="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl rotate-12 animate-float"/>
                </div>
              </div>
            </div>
        ...
        <div class="w-full md:w-1/2 bg-white dark:bg-gray-800 p-8 md:p-12 flex flex-col justify-center">
            <div class="text-center mb-8">
              <div class="inline-flex items-center justify-center w-16 h-16 bg-teal-100 dark:bg-teal-900/30 rounded-2xl mb-4">
              ...
              <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                {{ currentStep === 'phone' ? 'ورود به داروپلاس' : 'تایید شماره موبایل' }}
              </h2>
              <p class="text-gray-600 dark:text-gray-400">
                {{ currentStep === 'phone' ? 'برای ادامه، شماره موبایل خود را وارد کنید' : 'کد تایید ارسال شده را وارد نمایید' }}
              </p>
            </div>

            <div v-if="currentStep === 'phone'" class="space-y-6">
              <div class="relative mb-12"> <!-- margin below to avoid overlap with next input -->
          ...
          'border-red-500 dark:border-red-500': phoneError
              ...
              <p class="text-sm text-center text-gray-600 dark:text-gray-400">
                ...
                <a href="#" class="text-teal-600 dark:text-teal-400 hover:underline">شرایط و قوانین</a>
                داروپلاس است.
              </p>
            </div>
            ...
            <div v-else-if="currentStep === 'otp'" class="space-y-6">
              ...
              <div class="p-4 rounded-xl" :class="isNewUser ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-green-50 dark:bg-green-900/20'">
                <p class="text-sm text-center" :class="isNewUser ? 'text-blue-700 dark:text-blue-300' : 'text-green-700 dark:text-green-300'">
                  {{ isNewUser ? '🎉 خوش آمدید! برای تکمیل ثبت‌نام کد را وارد کنید' : '✅ کد تایید به شماره ' + phoneNumber + ' ارسال شد' }}
                </p>
              </div>
              ...
              <div>
                ...
                <p v-if="otpError" class="mt-2 text-sm text-center text-red-600 dark:text-red-400">
                  {{ otpError }}
                </p>
              </div>

              <div class="text-center">
                <p v-if="resendTimer > 0" class="text-sm text-gray-600 dark:text-gray-400">
              ...
              >
                <span v-if="!isLoading">{{ isNewUser ? 'تکمیل ثبت‌نام' : 'ورود' }}</span>
                <span v-else class="flex items-center justify-center gap-2">
                  <svg class="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
...
export default {
  ...
  methods: {
    validatePhone() {
      ...
      if (this.phoneNumber.length === 11 && !/^09\d{9}$/.test(this.phoneNumber)) {
